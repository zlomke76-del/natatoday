...
function getRole(job: AnyRow | null, application: AnyRow) {
  return asText(
    job?.title,
    asText(application.role, asText(application.job_title, "the role")),
  );
}
...
function getCandidateName(application: AnyRow) {
  return asText(
    ...
    asText(
      ...
      asText(application.full_name, "the candidate"),
    ),
  );
}
...
function getCandidatePhotoUrl(application: AnyRow) {
  return asText(
    ...
    asText(
      ...
      asText(application.candidate_photo_url, asText(application.headshot_url)),
    ),
  );
}
...
function buildVerificationItems(role: string) {
  ...
  if (lower.includes("advisor")) {
    return [
      "Confirm service-lane experience",
      "Confirm DMS/process familiarity",
      ...common,
    ].slice(0, 5);
  }
...
function buildNataNotes({
  ...
  const fitScore = application.fit_score
    ? ` Fit score: ${application.fit_score}.`
    : "";
  const notesLine = virtualNotes
    ? ` Virtual interview notes: ${virtualNotes}`
    : "";
  const summaryLine =
    screeningSummary ||
    coverNote ||
    "Candidate has been reviewed for manager handoff.";
...
export async function generateInterviewPacket(applicationId: string) {
  ...
  const candidatePhotoUrl = getCandidatePhotoUrl(application);
  ...
  if (packetError || !packet) {
    throw new Error(
      packetError?.message || "Interview packet could not be generated",
    );
  }
