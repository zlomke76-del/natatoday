...
export default async function RecruiterDashboard({
  ...
  const openJobs = jobs.filter((job) => !job.filled_at && job.publish_status !== "closed");

  const dealers = Array.from(
    new Set(openJobs.map((job) => label(job.dealer_slug || job.public_dealer_name, "unknown-dealer")))
  ).map((dealerSlug) => {
    const dealerJobs = openJobs.filter(
      (job) => label(job.dealer_slug || job.public_dealer_name, "unknown-dealer") === dealerSlug
    );
    ...
    const readyForDealer = dealerApps.filter(
      (app) => app.status === "dealer_interview_scheduled" && app.dealer_interview_at && app.interview_packet_ready
    );
  ...
  const needsInterview = applications.filter((app) => !app.virtual_interview_completed_at);
    ...
    <main className="shell">
      ...
      <section style={{ width: "min(1180px, calc(100% - 40px))", margin: "0 auto", padding: "60px 0" }}>
        ...
        <p style={{ color: "#cfe2ff", maxWidth: 820 }}>
          Daily visibility for dealer demand, open jobs, candidate movement, interviews, packet readiness, and dealer handoff status.
        </p>
        ...
        <div style={{ display: "grid", gap: 14 }}>
          {dealers.length === 0 ? (
            <div style={{ padding: 24, borderRadius: 20, background: "rgba(255,255,255,0.05)", color: "#cfe2ff" }}>
              ...
              >
                ...
                <span
                  style={{
                    ...
                    padding: "8px 12px",
                    borderRadius: 999,
                    background: dealer.priority === "Action" ? "#1473ff" : "rgba(255,255,255,0.08)",
                    fontWeight: 900,
                  }}
                >
                  {dealer.priority}
        ...
        >
          {[
            ...
            >
              <div style={{ color: "#fbbf24", fontWeight: 900, letterSpacing: ".12em", textTransform: "uppercase", fontSize: 12 }}>
                {title}
              </div>
                ...
                >
                  <div>
                    ...
                    <p style={{ margin: "10px 0 0", color: "#9fb1cc" }}>
                      Status: {application.status || "new"} · Virtual: {application.virtual_interview_status || "not_scheduled"} · Packet:{" "}
                      {application.interview_packet_ready ? "ready" : "not ready"} · Dealer: {formatDate(application.dealer_interview_at)}
                    </p>

                    <p style={{ margin: "10px 0 0", color: "#fbbf24", fontWeight: 800 }}>
