      ...
      <section style={wrapStyle}>
        ...
        <div style={heroGridStyle}>
          <div>
            ...
            <p style={ledeStyle}>
              Invite recruiters and agents, control execution permissions, and assign candidate work without changing the underlying hiring states.
            </p>
          </div>
          ...
          <div style={principleCardStyle}>
            ...
            <p style={principleTextStyle}>
              Don keeps full visibility. Recruiters and agents remain invited until they accept. Assigned work controls what they see.
            </p>
          </div>
        </div>
        ...
        <div style={mainGridStyle}>
          <section style={panelStyle}>
            <div style={eyebrowStyle}>Invite team member</div>
            ...
            <form method="POST" action="/api/nata/recruiters/invite" style={formStyle}>
              <div style={formGridStyle}>
                ...
                <Field label="Email" name="email" type="email" placeholder="brandy@natatoday.ai" required />
              ...
              <div style={noticeStyle}>
                Invites create or update the recruiter row as <strong>invited</strong>, generate a secure token, and send onboarding by email/SMS when contact channels exist.
              </div>
          ...
          <div style={assignmentListStyle}>
            {applications.length === 0 ? (
              ...
              applications.map((application) => {
                ...
                const dealerName = application.jobs?.public_dealer_name || application.jobs?.dealer_slug || "Dealer pending";
                ...
                return (
                  <article key={application.id} style={assignmentCardStyle}>
                    <div>
                      ...
                      <p style={candidateMetaStyle}>{roleTitle} · {dealerName}</p>
...
function Metric({ label, value }: { label: string; value: number }) {
  return <div style={metricCardStyle}><div style={metricValueStyle}>{value}</div><div style={metricLabelStyle}>{label}</div></div>;
}
...
function Field({ label, name, type = "text", placeholder, required = false }: { label: string; name: string; type?: string; placeholder?: string; required?: boolean }) {
  return <label style={labelStyle}><span>{label}</span><input name={name} type={type} placeholder={placeholder} required={required} style={inputStyle} /></label>;
}
...
function RosterGroup({ title, recruiters, applications, allRecruiters, loadSnapshots }: { title: string; recruiters: Recruiter[]; applications: AssignmentRow[]; allRecruiters: Recruiter[]; loadSnapshots: LoadSnapshot[] }) {
  return (
    <div style={rosterGroupStyle}>
      ...
      {recruiters.length === 0 ? <div style={emptyRosterStyle}>None yet.</div> : recruiters.map((recruiter) => {
        const role = normalizeRole(recruiter.role);
        ...
        return (
          <article key={recruiter.id} style={rosterCardStyle}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <p style={rosterMetaStyle}>{ROLE_LABELS[role] || role} · {recruiter.title || "No title"}</p>
              <p style={rosterMetaStyle}>{recruiter.email || "No email"} · {recruiter.phone || "No phone"} · Manager: {manager?.name || "None"}</p>
...
const wrapStyle: CSSProperties = { width: "min(1320px, calc(100% - 40px))", margin: "0 auto" };
...
const heroGridStyle: CSSProperties = { marginTop: 28, display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(280px, 380px)", gap: 24, alignItems: "end" };
...
const ledeStyle: CSSProperties = { color: "#bfd6f5", maxWidth: 780, fontSize: 18, lineHeight: 1.6 };
...
const mainGridStyle: CSSProperties = { display: "grid", gridTemplateColumns: "minmax(390px, 0.8fr) minmax(0, 1.2fr)", gap: 18, marginTop: 36, alignItems: "start" };
...
const noticeStyle: CSSProperties = { padding: 12, borderRadius: 14, background: "rgba(37,99,235,0.12)", border: "1px solid rgba(96,165,250,0.24)", color: "#dbeafe", fontSize: 13, lineHeight: 1.45 };
