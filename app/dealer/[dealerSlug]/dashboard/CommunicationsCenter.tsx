...
function getMessageDirection(message: AnyRow) {
...
function getMessageStatus(message: AnyRow) {
...
export default async function CommunicationsCenter({
  ...
  const inbox = messages.filter((message) => getMessageDirection(message) === "inbound");
  const outbox = messages.filter((message) => getMessageDirection(message) === "outbound");
    ...
    >
      ...
      >
        <div>
          ...
          >
            Inbox and outbox activity tied to this dealership. Candidate and
            recruiter messages stay attached to the hiring workflow instead of
            disappearing into personal inboxes.
          </p>
        </div>
      ...
      {messages.length === 0 ? (
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {messages.slice(0, 8).map((message) => (
            <MessageCard
              key={String(message.id || `${mode}-${message.created_at}-${message.subject}`)}
              message={message}
              mode={mode}
            />
          ))}
...
function MessageCard({
  ...
  return (
    ...
    >
      ...
      >
        {getMessageBody(message).slice(0, 280)}
        {getMessageBody(message).length > 280 ? "…" : ""}
      </p>
